...
- We learned in the last class how to filter rows based on a single condition using \texttt{filter}. For today, let's use the \texttt{nycflights13::flights} dataframe. This dataframe contains lots of information about flights departing NYC in 2013.
...
- Notice here, the ampersand serves as the "and" operator. If we use "and" then this means that \texttt{filter} will only select those rows which meet \textit{\underline{both}} conditions.
...
```{=tex}
 \begin{table}[h]
    ...
    \texttt{|} & \text{Or} & \text{\texttt{month == 1 | month == 2}} \\
...
- Occassionally, we need to know how many distinct rows we have in a dataset. To do this, we can make use of the \texttt{distinct} function.

\vskip 0.15 in

- Suppose we want to remove duplicate rows, if any exist, from the \texttt{flights} dataframe:
...
- Notice in the prior example, only the columns \texttt{origin} and \texttt{dest} were retained.
...
- \texttt{contains("arr")} selects those columns whose names match the regular expression specified in the function
...
```{r,echo=T,out.width="75%",out.height="75%",fig.align="center"}
stdize <- pos_skew |>
